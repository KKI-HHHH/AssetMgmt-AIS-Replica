      ...
      <div className="bg-slate-50 w-full max-w-5xl h-[80vh] rounded-lg shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
        ...
        <div className="p-6 bg-white border-b border-slate-200 rounded-t-lg flex justify-between items-center">
            <div className="flex items-center gap-3">
                <div className="p-2 bg-siteblue/10 rounded-lg text-siteblue">
                    <FileSpreadsheet size={24} />
                </div>
                <div>
                    <h2 className="text-xl font-bold text-siteblue">Import Data Tool</h2>
        ...
        <div className="bg-white border-b border-slate-200 px-6 py-4">
            <div className="flex items-center justify-center">
                <div className={`flex items-center gap-2 ${step >= 1 ? 'text-siteblue font-semibold' : 'text-slate-400'}`}>
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step >= 1 ? 'bg-siteblue text-white' : 'bg-slate-200'}`}>1</span>
                    Input
                </div>
                <div className={`w-12 h-0.5 mx-2 ${step >= 2 ? 'bg-siteblue' : 'bg-slate-200'}`}></div>
                <div className={`flex items-center gap-2 ${step >= 2 ? 'text-siteblue font-semibold' : 'text-slate-400'}`}>
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step >= 2 ? 'bg-siteblue text-white' : 'bg-slate-200'}`}>2</span>
                    Map Columns
                </div>
                <div className={`w-12 h-0.5 mx-2 ${step >= 3 ? 'bg-siteblue' : 'bg-slate-200'}`}></div>
                <div className={`flex items-center gap-2 ${step >= 3 ? 'text-siteblue font-semibold' : 'text-slate-400'}`}>
                    <span className={`w-6 h-6 rounded-full flex items-center justify-center text-xs ${step >= 3 ? 'bg-siteblue text-white' : 'bg-slate-200'}`}>3</span>
                    Preview & Import
                </div>
            </div>
        </div>
                ...
                <div className="h-full flex flex-col gap-6">
                    <div className="grid grid-cols-2 gap-6">
                        <div>
                            ...
                            <select 
                                ...
                                className="w-full border border-slate-300 rounded-md p-2.5 shadow-sm focus:ring-siteblue focus:border-siteblue"
                            >
                        ...
                        <div className="flex items-end mb-3">
                             <label className="flex items-center gap-2 cursor-pointer">
                                <input type="checkbox" checked={hasHeader} onChange={e => setHasHeader(e.target.checked)} className="rounded text-siteblue w-4 h-4" />
                    ...
                    <div className="flex-grow flex flex-col">
                        ...
                        <textarea 
                            ...
                            className="flex-grow w-full border border-slate-300 rounded-md p-4 font-mono text-sm shadow-sm focus:ring-siteblue focus:border-siteblue resize-none"
                        />
                    </div>
                </div>
            )}
                            ...
                            <div key={index} className="flex items-center gap-4 p-4 bg-white border border-slate-200 rounded-lg shadow-sm">
                                ...
                                <div className="flex-grow">
                                    ...
                                    <select 
                                        ...
                                        className={`w-full p-2 border rounded-md text-sm ${columnMapping[index] ? 'border-siteblue/30 bg-siteblue/5 text-siteblue font-medium' : 'border-slate-300 text-slate-600'}`}
                                    >
                                        ...
                                        {schema.map(field => (
                                            <td key={field.key} className="px-3 py-2 text-sm border-r border-transparent hover:border-slate-200">
                                                <input 
                                                    ...
                                                    className={`w-full bg-transparent focus:outline-none focus:bg-siteblue/5 rounded px-1 ${field.required && !row[field.key] ? 'border-b-2 border-red-300 bg-red-50' : ''}`}
                                                />
                                            </td>
                                        ))}
                                    </tr>
        ...
        {/* Footer Actions */}
        <div className="p-4 bg-[rgb(248,249,250)] border-t border-slate-200 flex justify-between items-center rounded-b-lg">
            <div className="text-xs text-disabledgrey">Import Task {step}/3</div>
            <div className="flex gap-3">
                {step === 1 && (
                    <>
                        <button onClick={onClose} className="px-4 py-2 text-sm font-semibold text-textblack bg-white border border-bordergrey rounded hover:bg-rowgrey">Cancel</button>
                        <button onClick={handleParse} disabled={!rawData} className="px-6 py-2 bg-siteblue text-white rounded-md text-sm font-semibold hover:opacity-90 disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2">
                ...
                {step === 2 && (
                    <>
                        <button onClick={() => setStep(1)} className="px-4 py-2 text-sm font-semibold text-textblack bg-white border border-bordergrey rounded hover:bg-rowgrey">Back</button>
                        <button onClick={handleMapConfirm} className="px-6 py-2 bg-siteblue text-white rounded-md text-sm font-semibold hover:opacity-90 flex items-center gap-2">
                ...
                {step === 3 && (
                    <>
                        <button onClick={() => setStep(2)} className="px-4 py-2 text-sm font-semibold text-textblack bg-white border border-bordergrey rounded hover:bg-rowgrey">Back</button>
                        <button onClick={handleFinalImport} className="px-6 py-2 bg-successgreen text-white rounded-md text-sm font-semibold hover:opacity-90 flex items-center gap-2">
