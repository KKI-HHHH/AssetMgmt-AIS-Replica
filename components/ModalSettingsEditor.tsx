            ...
            <div className="p-4 bg-white border-b border-slate-200 flex justify-between items-center">
                <div className="flex items-center gap-4">
                    ...
                    <select 
                        ...
                        className="text-sm border-slate-300 rounded-md shadow-sm focus:border-siteblue focus:ring-siteblue"
                    >
                        {CONTEXT_OPTIONS.map(opt => <option key={opt.key} value={opt.key}>{opt.label}</option>)}
                    </select>
                </div>
                <button onClick={handleSave} className="flex items-center gap-2 px-4 py-2 bg-siteblue text-white rounded-md text-sm font-semibold hover:opacity-90 shadow-sm">
                    ...
                    <div className="overflow-y-auto p-2 space-y-2 flex-grow">
                        {availableFields.map(field => (
                            <div 
                                ...
                                className="flex items-center gap-2 p-2 bg-white border border-slate-200 rounded shadow-sm cursor-grab hover:border-siteblue/40 active:cursor-grabbing"
                            >
                ...
                <div className="flex-grow flex flex-col bg-slate-100 overflow-hidden">
                    ...
                    <div className="p-4 border-b border-slate-200 bg-white flex gap-2 items-center overflow-x-auto">
                        {currentLayout.tabs.map(tab => (
                            <div key={tab.id} className={`flex items-center group px-3 py-2 rounded-t-md border-b-2 transition-colors cursor-pointer ${activeTabId === tab.id ? 'border-siteblue bg-siteblue/5 text-siteblue' : 'border-transparent hover:bg-slate-50'}`} onClick={() => setActiveTabId(tab.id)}>
                                <input 
                                    value={tab.label}
                            ...
                            <div className="space-y-6 max-w-4xl mx-auto">
                                {currentTab.sections.map((section, index) => (
                                    <div 
                                        ...
                                        className={`bg-white rounded-lg border-2 ${draggedField ? 'border-dashed border-siteblue/40' : 'border-slate-200'} p-4 transition-colors`}
                                    >
                                        {/* Section Header */}
                                        <div className="flex justify-between items-center mb-4 border-b border-slate-100 pb-2">
                                            <div className="flex items-center gap-2">
                                                <LayoutGrid size={16} className="text-siteblue"/>
                                                <input 
                                                    value={section.title}
                                        ...
                                        <div className={`grid gap-4 min-h-[50px] ${section.fields.length === 0 ? 'bg-slate-50 rounded border-2 border-dashed border-slate-200 flex items-center justify-center' : ''}`} style={{ gridTemplateColumns: `repeat(${section.columns}, minmax(0, 1fr))` }}>
                                            {section.fields.map(field => (
                                                <div 
                                                    ...
                                                    onDragEnd={handleDragEnd}
                                                    className="flex justify-between items-center p-2 bg-slate-50 border border-slate-200 rounded group cursor-grab active:cursor-grabbing hover:border-siteblue/40"
                                                >
                                ...
                                <button onClick={() => addSection(currentTab.id)} className="w-full py-3 border-2 border-dashed border-slate-300 rounded-lg text-slate-500 hover:border-siteblue/60 hover:text-siteblue hover:bg-siteblue/5 transition-colors flex items-center justify-center gap-2">
