...
import { Settings, ChevronDown, FilePenLine, Plus, Layers, Package, Download } from 'lucide-react';
...
const InfoPair: React.FC<{ label: string, value: string | number | undefined }> = ({ label, value }) => (
    <div>
        <p className="text-xs text-disabledgrey font-medium">{label}</p>
        <p className="text-sm font-semibold text-textblack">{value || '-'}</p>
    </div>
);

const VariantCard: React.FC<{ variant: LicenseVariant }> = ({ variant }) => (
    <div className="bg-white rounded-md p-3 border border-bordergrey">
        <h4 className="font-semibold text-siteblue">{variant.name}</h4>
        <div className="mt-2 text-xs text-textblack space-y-1">
...
const FormRadioGroup: React.FC<{
  ...
  <div>
    <label className="block text-sm font-medium text-textblack mb-2">{label}</label>
    <div className="flex flex-wrap gap-2">
      {options.map((option) => (
        <label key={option.value} className="relative">
          ...
          />
          <span className="block px-4 py-2 text-sm border rounded-full cursor-pointer peer-checked:bg-siteblue peer-checked:text-white peer-checked:border-siteblue bg-white border-bordergrey hover:bg-rowgrey transition-colors">
            {option.label}
          </span>
        </label>
      ))}
...
const BulkCreateModal: React.FC<{
  ...
  return (
    <div className="fixed inset-0 z-50 flex justify-center items-center p-4 bg-black/40 backdrop-blur-sm" onClick={onClose}>
      <div className="bg-bggrey rounded-lg shadow-xl w-full max-w-md" onClick={e => e.stopPropagation()}>
        <form onSubmit={handleSubmit}>
            <div className="p-6 border-b bg-white rounded-t-lg">
                <h2 className="text-lg font-bold text-siteblue">Bulk Create Licenses for {family.name}</h2>
            </div>
            <div className="p-6 space-y-4">
                ...
                />
                <div>
                    <label className="block text-sm font-medium text-textblack mb-1">Quantity</label>
                    <input type="number" value={quantity} onChange={e => setQuantity(parseInt(e.target.value, 10))} min="1" className="w-full p-2 border border-bordergrey rounded" placeholder="Quantity"/>
                </div>
                 <div>
                    <label className="block text-sm font-medium text-textblack mb-1">Purchase Date</label>
                    <input type="date" value={purchaseDate} onChange={e => setPurchaseDate(e.target.value)} className="w-full p-2 border border-bordergrey rounded" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-textblack mb-1">Renewal Date</label>
                    <input type="date" value={renewalDate} onChange={e => setRenewalDate(e.target.value)} className="w-full p-2 border border-bordergrey rounded" />
                </div>
                <div>
                    <label className="block text-sm font-medium text-textblack mb-1">Cost per License</label>
                    <input type="number" step="0.01" value={cost} onChange={e => setCost(parseFloat(e.target.value))} className="w-full p-2 border border-bordergrey rounded" placeholder="Cost per license"/>
                </div>
            </div>
            <div className="p-4 bg-[rgb(248,249,250)] border-t flex justify-end gap-3 rounded-b-lg">
                <button type="button" onClick={onClose} className="px-4 py-2 bg-white border border-bordergrey rounded-md text-sm font-semibold hover:bg-rowgrey">Cancel</button>
                <button type="submit" className="px-4 py-2 bg-siteblue text-white rounded-md text-sm font-semibold hover:opacity-90">Create Licenses</button>
            </div>
        </form>
      </div>
    </div>
...
const AssetProfile: React.FC<AssetProfileProps> = ({ family, allAssets, onBack, onEditAsset, onEditFamily, onAddInstance, onBulkCreate, onUserClick }) => {
  ...
  const columns: ColumnDef<Asset>[] = [
    ...
    ...(isSoftware ? [
        ...
        { 
          ...
          cell: ({ row }: {row: {original: Asset}}) => {
            ...
            if (users.length === 0) return <span className="text-disabledgrey">-</span>;
            return (
              <div className="flex flex-col">
                {users.map(user => (
                   <button key={user.id} onClick={() => onUserClick(user)} className="text-siteblue hover:underline truncate text-left">
                     {user.fullName}
                   </button>
                ))}
              </div>
        ...
        { 
            ...
            cell: ({row}) => {
                ...
                const dateStr = formatDisplayDate(row.original.renewalDate);
                return (
                    <div>
                        <div className="text-textblack">{dateStr}</div>
                        {period && <div className="text-xs text-siteblue font-medium bg-sitebluelight inline-block px-1 rounded">{period}</div>}
                    </div>
                )
            }
        },
        ...
        { 
          ...
          cell: ({ row }: {row: {original: Asset}}) => row.original.assignedUser ? (
            <button onClick={() => onUserClick(row.original.assignedUser!)} className="font-medium text-siteblue hover:underline truncate block w-full text-left">
        ...
        { accessorKey: 'warrantyExpiryDate', header: 'Warranty Ends', width: 140, cell: ({row}) => <span>{formatDisplayDate(row.original.warrantyExpiryDate)}</span> },
        { accessorKey: 'purchaseDate', header: 'Purchase Date', width: 140, cell: ({row}) => <span>{formatDisplayDate(row.original.purchaseDate)}</span> },
    ...
    ) },
    {
      ...
      width: 60,
      cell: ({ row }) => (
        <div className="flex items-center justify-end gap-2 text-siteblue">
          <button onClick={() => onEditAsset(row.original)} className="p-1 hover:opacity-80 rounded-full hover:bg-rowgrey">
    ...
    <div className="flex items-center gap-2">
      {isSoftware && (
        <button 
            ...
            className="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-white bg-disabledgrey rounded-md shadow-sm hover:opacity-90">
            <Layers size={16} /> Bulk Create
        </button>
      )}
      <button 
        onClick={() => onAddInstance(family)}
        className="flex items-center gap-2 px-3 py-2 text-sm font-semibold text-white bg-siteblue rounded-md shadow-sm hover:opacity-90">
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-bggrey min-h-screen">
       <div className="max-w-full mx-auto">
            <div className="mb-4 text-sm text-disabledgrey">
                <button onClick={onBack} className="font-medium text-siteblue hover:underline">Asset Repository</button>
                <span className="mx-2">/</span>
                <span className="font-medium text-textblack">{family.name}</span>
            </div>
            
            <div className="flex flex-col lg:flex-row gap-6">
                {/* Left Panel */}
                <div className="lg:w-1/3 xl:w-1/4 flex-shrink-0 space-y-6">
                    <div className="bg-white p-5 rounded-lg shadow-sm border border-bordergrey">
                        <div className="flex justify-between items-start">
                           <h2 className="text-xl font-bold text-siteblue flex items-center gap-2">
                             {isSoftware ? <Settings size={22} className="text-siteblue" /> : <Package size={22} className="text-successgreen" />}
                             {family.name}
                           </h2>
                           <button onClick={() => onEditFamily(family)} className="p-1 text-siteblue hover:opacity-80 hover:bg-rowgrey rounded-full"><CustomEditIcon size={16} /></button>
                        </div>
                        ...
                        {family.description && (
                            <div className="mt-4 pt-4 border-t border-bordergrey">
                                <p className="text-xs text-disabledgrey font-medium">Description</p>
                                <p className="text-sm text-textblack">{family.description}</p>
                            </div>
                        )}
                    </div>
                     {isSoftware && (
                        <div className="bg-white p-5 rounded-lg shadow-sm border border-bordergrey">
                            <h3 className="text-base font-bold text-textblack mb-3">License Variants</h3>
