    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 z-50 flex justify-center items-center p-4">
      <div className="bg-white rounded-lg shadow-xl w-full max-w-lg max-h-[90vh] flex flex-col">
        <div className="p-6 border-b border-slate-200 flex justify-between items-center">
          <h2 className="text-xl font-bold text-siteblue">New Asset Request{category && `: ${category}`}</h2>
        ...
        <div className="p-6 overflow-y-auto">
            ...
            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 mb-4">
                {searchedFamilies.length > 0 ? searchedFamilies.map(family => (
                    <div 
                        ...
                        className={`p-3 rounded-md border cursor-pointer transition-all duration-150 ${selectedFamilyId === family.id ? 'bg-siteblue/5 border-siteblue ring-2 ring-siteblue/30' : 'bg-white border-slate-200 hover:bg-slate-50'}`}
                    >
            ...
            <div>
              ...
              <textarea
                ...
                className="w-full p-2 border border-slate-300 rounded-md text-sm focus:ring-siteblue focus:border-siteblue"
              />
            </div>
        </div>

        <div className="p-6 bg-[rgb(248,249,250)] border-t border-slate-200 flex justify-end items-center gap-3">
            <button type="button" onClick={onClose} className="px-4 py-2 text-sm font-medium text-slate-700 bg-white border border-slate-300 rounded-md shadow-sm hover:bg-slate-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-siteblue">
              Cancel
            </button>
            <button 
                type="button" 
                ...
                className="px-4 py-2 text-sm font-medium text-white bg-siteblue border border-transparent rounded-md shadow-sm hover:opacity-90 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-siteblue disabled:bg-slate-300 disabled:cursor-not-allowed"
            >
              Submit Request
            </button>
          </div>
